...
def load_pretrained_weights(model, pretrained_weights, checkpoint_key=None):
    if os.path.isfile(pretrained_weights):
        ...
        #state_dict = {k.replace("backbone.", "module.backbone."): v for k, v in state_dict.items()}
...
class MultiCropWrapper(nn.Module):
    ...
    def forward(self, images, events, voxel, mask_ratio):
        # convert to list t5encodemodel = None

        loss_rgb_mae, loss_event_mae, loss_fuse_two_eve_mae, loss_e_r, loss_fuse_three_rgb_mae , loss_e_v = self.backbone(images, events, voxel, mask_ratio)#, loss_e_v

        return loss_rgb_mae, loss_event_mae, loss_fuse_two_eve_mae, loss_e_r, loss_fuse_three_rgb_mae , loss_e_v #, loss_fuse_three_event_mae , loss_e_v
